...
namespace TestUnits.TextBuilder
{
    internal class arquivo
    {
        ...
        public static class TextBuilder
        {
                ...
                for (; patIndex < matchSpanText.Length; patIndex++)
                {
                    if (matchSpanText[patIndex] == '*' || patIndex == matchSpanText.Length - 1)
                    {
                        ...
                        if (!occur.Empty)
                        {
                            ...
                            else { occurLen += occur.Word.Length; }// (occur.Position - occurIni) + occur.Word.Length; }
            ...
            public static string ExtractFirstSnippet(ReadOnlySpan<char> text, string snippetTags, TextOptions options = default)
            {
                string[] _occurs;

                _occurs = snippets(text, "", snippetTags, 1, options);
            ...
            private static string[] snippets(ReadOnlySpan<char> text, string snippet, string openCloseTags = "", int count = 0, TextOptions options = default)
            {
